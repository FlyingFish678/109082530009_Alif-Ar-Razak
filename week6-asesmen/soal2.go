...
func perhitunganBiaya(jumlahMhs, lamaPerjalanan int, tujuan string, totalBiaya *float64) {
	...
	totalPerHariDomestik := biayaPerHari(jumlahMhs)

	if tujuan == "domestik" {
		*totalBiaya = float64(hariDitanggung * totalPerHariDomestik)
	} else if tujuan == "mancanegara" {
		*totalBiaya = float64(hariDitanggung*totalPerHariDomestik) * 1.5
	}
}
